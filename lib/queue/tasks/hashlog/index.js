...
module.exports = function hashlog (doc) {
  let workdir = path.dirname(doc.path)
  return Promise.promisify(fs.stat)(doc.path)
  .then((stats) => {
    let hashlogPath = path.join(workdir, 'hashlog.md5')
    if (doc.path.endsWith('E01')) {
  ...
  .then((isdone) => {
    if (!isdone) {
      let promise = spawn(path.join(__dirname, 'hash/parallelhash.sh'), [doc.path])
